...
- **A missing variable has to fail at boot.** Asserting `process.env.DATABASE_URL!` defers the failure to the first query, where it surfaces as a driver error naming neither the variable nor the cause. Refusing to start says what is wrong, once.
- **A liveness check must not test the database, and a deploy must not gate on liveness.** A check that reports the process's own state answers `200` while the database is unreachable, so a deploy gated on it reports success over a release that cannot serve. Making that one check dependency-aware is the wrong fix — it pulls every instance out of rotation over a blip. The two questions need two endpoints: [api.md](api.md#liveness-and-readiness).
- **A statement timeout is not optional.** A query that never returns holds its pool connection indefinitely, and enough of them exhaust the pool. `statement_timeout` and `query_timeout` belong in the pool configuration, sized above the slowest legitimate query.
